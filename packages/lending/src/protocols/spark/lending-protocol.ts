...
export class LendingProtocol extends Protocol {
  ...
  async initializeReservesConfig() {
    ...
    try {
      reserveTokens = await fetchReservesData(this.id, this.chainId);
    } catch (error) {
      const service = new logics.spark.Service(this.chainId, this.provider);
      ({ reserveTokens } = await service.getReserveTokens());
    }
  ...
  async getBorrowTokenList() {
    if (!this._borrowTokenList) {
      this._borrowTokenList = await apisdk.protocols.spark.getBorrowTokenList(this.chainId);
    }
